...
public class DynamicMeshManager : MonoBehaviour
{
    ...
    public void OnGUI()
    {
		...
		if (GUI.Button(new Rect(Screen.width - 320, 20, 140, 80), "Toggle Mesh"))
		{
			m_meshingCubePrefab.GetComponent<Renderer>().material.shader = (m_meshingCubePrefab.GetComponent<Renderer>().material.shader == Shader.Find("Legacy Shaders/Diffuse"))
				? Shader.Find("UI/Unlit/Transparent") : Shader.Find("Legacy Shaders/Diffuse");
		}
		if (GUI.Button(new Rect(Screen.width - 320, 120, 140, 80), "Toggle Video"))
		{
			Camera.main.GetComponent<Renderer>().material.shader =
				(Camera.main.GetComponent<Renderer>().material.shader == Shader.Find("UI/Unlit/Transparent"))
				? Shader.Find("Tango/YUV2RGB") : Shader.Find("UI/Unlit/Transparent");
		}
    }
